        ...
        switch (dragEvent.getAction()) {
            ...
            case DragEvent.ACTION_DROP:
                ...
                if (view.getId() == R.id.flRecycleItem || view.getId() == R.id.TVAllItemListEmpty
                    ...
                    ItemObject list = adapterSource.getList().get(positionSource);
                    ...
                    if (positionTarget >= 0) {
                        ...
                        Bitmap img = ((BitmapDrawable) list.getAppIcon()).getBitmap();
                        ByteArrayOutputStream stream = new ByteArrayOutputStream();
                        img.compress(Bitmap.CompressFormat.PNG, 100, stream);
                        byte[] imageInByte = stream.toByteArray();
                       String image = Base64.encodeToString(imageInByte, Base64.DEFAULT);

                        log_list.add(positionTarget, new program(null,list.getAppName(),image,list.getPackagename()));
                        ...
                        byte[] imageInByte = stream.toByteArray();
                        String image = Base64.encodeToString(imageInByte, Base64.DEFAULT);
                        log_list.add(new program(null,list.getAppName(),image,list.getPackagename()));
                    }
